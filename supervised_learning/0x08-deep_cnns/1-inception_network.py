...
def inception_network():
    ...
    layer = K.layers.Conv2D(64, (7, 7), strides=2, padding='same', activation='relu')(input_layer)
    layer = K.layers.MaxPooling2D(
        (3, 3), strides=2, padding='same')(layer)
    layer = K.layers.Conv2D(64, (3, 3), strides=1, padding='same', activation='relu')(layer)
    layer = K.layers.Conv2D(192, (3, 3), strides=1, padding='same', activation='relu')(layer)
    layer = K.layers.MaxPooling2D(
        (3, 3), strides=2, padding='same')(layer)
    ...
    layer = K.layers.AveragePooling2D((7, 7), strides=1, padding='valid')(layer)
    layer = K.layers.Dropout(0.4)(layer)
    Y = K.layers.Dense(1000, activation='softmax')(layer)
